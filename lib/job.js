...
export default class Job {
   ...
   async findById(id) {
      let results = this.pool.query(JOB_DETAILS_QUERY, [id]);
      return (results.length)? results[0] : null;
   }

   async exists(name) {
      let results = this.pool.query('SELECT * FROM job where name = ?', [name]);
   ...
   async create(name) {
      ...
      await this.pool.query('INSERT IGNORE INTO job SET name = ?', [name]);
      return true;
   }
}
