   ...
   */
   ...
   async updateDem(id, dem) {
      return new Promise((resolve, reject) => {
         // INSERT IGNORE INTO journal (`id`, `coordinates`) VALUES (NULL, GeomFromText('POINT(40.782710 -73.965310)'));
         this.pool.getConnection(function (err, connection) {
            connection.query('UPDATE journal SET ? WHERE id = ' + id, {dem}, function (error, results, fields) {
               connection.release();
               if (error) reject(error);
               else resolve(results, fields);
            });
   ...
   async all() {
      return new Promise((resolve, reject) => {
         this.pool.getConnection(function (err, connection) {
            connection.query('SELECT * FROM journal order by time_point', function (error, results, fields) {
               connection.release();
               if (error) reject(error);
               else {
                  resolve(results);
   ...
   async pageNeedingDem(count = 2000) {
      return new Promise((resolve, reject) => {
         this.pool.getConnection(function (err, connection) {
            connection.query('SELECT * FROM journal where dem < -9000  order by id desc limit ?', [count], function (error, results, fields) {
               connection.release();
               if (error) reject(error);
               else {
                  resolve(results);
