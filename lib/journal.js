...
export default class Journal {
   ...
   */
   ...
   async countsForDay(date) {
      let dateStr = date.getFullYear() + "-" + String(date.getMonth() + 1).padStart(2, 0) + "-" + String(date.getDate()).padStart(2, 0);
      let results = await this.pool.query('SELECT COUNT(*) as points, job_id FROM journal WHERE DATE(time_point) = ? GROUP BY job_id ', [dateStr]);
      return results[0];
   }


   async pageNeedingDem(count = 2000) {
      let results = await this.pool.query('SELECT * FROM journal where dem < -900  order by id asc limit ?', [count]);
      return results[0];
   }
}
