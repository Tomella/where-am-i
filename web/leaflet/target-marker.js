...
const angleCrds = (map, nextLatlng, latlng) => {
    if (!latlng || !nextLatlng) return 0;
    const pxStart = map.project(latlng);
    const pxEnd = map.project(nextLatlng);
    return Math.atan2(pxStart.y - pxEnd.y, pxStart.x - pxEnd.x) / Math.PI * 180 - 90;
};
...
const TargetMarker = L.CircleMarker.extend({
    _updatePath() {
        if (!this.options.img || !this.options.img.url) return;
        if (!this.options.img.el) {
            this.options.img = { ...defaultImgOptions, ...this.options.img };
            this.options.img.rotate += angleCrds(this._map, this.options.nextLatlng, this._latlng);
            const img = document.createElement('img');
            img.src = this.options.img.url;
            this.options.img.el = img;
            img.onload = () => {
            ...
            this._renderer._updateImg = function (layer) {
                const { img } = layer.options;
                const p = layer._point.round();
                p.x += img.offset.x; p.y += img.offset.y;
                if (img.rotate) {
